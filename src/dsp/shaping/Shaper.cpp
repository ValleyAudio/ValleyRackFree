...
void Shaper::lean(const __m128& a, const __m128& f) {
    ...
    x = _mm_mul_ps(x, x);
    __output = _mm_linterp_ps(a, x, f);
}

void Shaper::lean2(const __m128& a, const __m128& f) {
    auto x = _mm_sub_ps(__ones, a);
    x = _mm_mul_ps(x, x);
    x = _mm_mul_ps(x, x);
    x = _mm_sub_ps(__ones, x);
    __output = _mm_linterp_ps(a, x, _mm_mul_ps(f, __minus));
}
...
void Shaper::twist(const __m128& a, const __m128& f) {
    ...
    __m128 x2 = _mm_add_ps(_mm_mul_ps(a, fScaled), _mm_mul_ps(_mm_sub_ps(fScaled, __ones), __minusHalf));
    __output = x1;
...
void Shaper::mirror(const __m128& a, const __m128& f) {
    ...
    auto x = _mm_sub_ps(_mm_mul_ps(a, __twos), __ones);
    x = _mm_mul_ps(x, _mm_add_ps(_mm_mul_ps(_mm_abs_ps(f), __nines), __ones));
...
void Shaper::sineWrap(const __m128& a, const __m128& f) {
    ...
    x = _mm_circle_ps(_mm_mul_ps(x, _mm_mul_ps(y, __eights)));
    __output = valley::_mm_sine_ps(_mm_mul_ps(x, _mm_set1_ps(M_PI)));
...
void Shaper::warble(const __m128& a, const __m128& f) {
    ...
    auto y = _mm_add_ps(a, _mm_mul_ps(__noise, f));
    y = _mm_circle_ps(_mm_sub_ps(_mm_mul_ps(y, __twos), __ones));
    __output = _mm_mul_ps(_mm_add_ps(y, __ones), __half);
...
void Shaper::buzzX2(const __m128&a, const __m128& f) {
    auto x = _mm_circle_ps(_mm_mul_ps(a, __twos));
    __output = _mm_add_ps(a, _mm_mul_ps(x, f));
}

void Shaper::buzzX4(const __m128&a, const __m128& f) {
    auto x = _mm_circle_ps(_mm_mul_ps(a, __fours));
    __output = _mm_add_ps(a, _mm_mul_ps(x, f));
}

void Shaper::buzzX8(const __m128&a, const __m128& f) {
    auto x = _mm_circle_ps(_mm_mul_ps(a, __eights));
    __output = _mm_add_ps(a, _mm_mul_ps(x, f));
}
...
void Shaper::sineDownX2(const __m128&a, const __m128& f) {
    auto x = _mm_mul_ps(_mm_sub_ps(__ones, a), f);
    auto y = _mm_circle_ps(_mm_mul_ps(a, __twos));
    y = valley::_mm_sine_ps(_mm_mul_ps(y, _mm_set1_ps(M_PI)));
    y = _mm_mul_ps(x, y);
    __output = _mm_add_ps(a, y);
    __output = _mm_circle_ps(_mm_sub_ps(_mm_mul_ps(__output, __twos), __ones));
    __output = _mm_mul_ps(_mm_add_ps(__output, __ones), __half);
}
...
void Shaper::sineDownX4(const __m128&a, const __m128& f) {
    auto x = _mm_mul_ps(_mm_sub_ps(__ones, a), f);
    auto y = _mm_circle_ps(_mm_mul_ps(a, __fours));
    y = valley::_mm_sine_ps(_mm_mul_ps(y, _mm_set1_ps(M_PI)));
    y = _mm_mul_ps(x, y);
    __output = _mm_add_ps(a, y);
    __output = _mm_circle_ps(_mm_sub_ps(_mm_mul_ps(__output, __twos), __ones));
...
void Shaper::sineDownX8(const __m128&a, const __m128& f) {
    auto x = _mm_mul_ps(_mm_sub_ps(__ones, a), f);
    auto y = _mm_circle_ps(_mm_mul_ps(a, __eights));
    y = valley::_mm_sine_ps(_mm_mul_ps(y, _mm_set1_ps(M_PI)));
    y = _mm_mul_ps(x, y);
    __output = _mm_add_ps(a, y);
    __output = _mm_circle_ps(_mm_sub_ps(_mm_mul_ps(__output, __twos), __ones));
